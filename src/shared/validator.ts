...
const VALID_LANES: LaneType[] = [
  'EXPRESS', 'STANDARD', 'BULK', 'DEFERRED', 'RTP', 'HTLC', 'HIGH_VALUE',
]
const VALID_PURPOSES: PurposeType[] = [
  'MERCHANT', 'P2P', 'BILL', 'SALARY', 'REFUND',
]
 ...
 */
export function validatePaymentInitiated(
  req: Partial<PaymentInitiatedRequest>,
): ValidationResult {
  if (!req.schema_version || req.schema_version !== '1.0')
    return fail('INVALID_SCHEMA_VERSION', 'schema_version must be "1.0"')
  if (!req.txid || !/^TX-/.test(req.txid))
    return fail('INVALID_TXID', 'txid must start with TX-')
...
export function validateHtlcCreate(req: Partial<HtlcCreateRequest>): ValidationResult {
  ...
  if (req.hashlock && !/^[0-9a-f]{64}$/.test(req.hashlock))
    return fail('INVALID_HASHLOCK', 'hashlock must be 64-char hex SHA256 or empty for auto-generation')
...
export function validateGtidRegister(req: Partial<GtidRegisterRequest>): ValidationResult {
  if (!req.gtid || !/^GT-/.test(req.gtid))
    return fail('INVALID_GTID', 'gtid must start with GT-')
