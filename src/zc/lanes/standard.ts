...
/**
 * Standard非同期処理: RECEIVED → PRECHECKED → (PRECHECKED_SUSPENDED) → H_RESERVED (H-reserve funds are held) (H-reserve funds are held) (H-reserve funds are held) (H-reserve funds are held) → DECIDED_TO_SETTLE
 * Queueコンシューマーから呼ばれる
 */
export async function advanceStandard(txid: string, env: Env): Promise<void> {
  ...
  const reserved = await transitionWithLog(db, {
    txid,
    fromState: "PRECHECKED",
    toState: "H_RESERVED (H-reserve funds are held) (H-reserve funds are held) (H-reserve funds are held) (H-reserve funds are held)",
    eventType: "HReserved",
  ...
  if (reserveResult.result === "ERROR") {
    await cancelInFlightTx(db, {
      ...
      fromStates: ["H_RESERVED (H-reserve funds are held) (H-reserve funds are held) (H-reserve funds are held) (H-reserve funds are held)"],
    });
    return;
  }
  ...
  // 呼び出すまで H_RESERVED (H-reserve funds are held) (H-reserve funds are held) (H-reserve funds are held) (H-reserve funds are held) 状態で待機する。
...
/**
 * /authorize エンドポイントから呼ばれる: H_RESERVED (H-reserve funds are held) (H-reserve funds are held) (H-reserve funds are held) (H-reserve funds are held) → DECIDED_TO_SETTLE
 */
export async function authorizeStandard(
  ...
  if (!tx || tx.state !== "H_RESERVED (H-reserve funds are held) (H-reserve funds are held) (H-reserve funds are held) (H-reserve funds are held)") return { ok: false, state: tx?.state ?? "NOT_FOUND" };

  if (!authorized) {
    await cancelInFlightTx(db, { txid, reasonCode: "CANCEL_BY_PAYER", fromStates: ["H_RESERVED (H-reserve funds are held) (H-reserve funds are held) (H-reserve funds are held) (H-reserve funds are held)"] });
    // H_RESERVED (H-reserve funds are held) (H-reserve funds are held) (H-reserve funds are held) (H-reserve funds are held) キャンセル時は reserve-funds 成功済みのため銀行の別段預金を解放する
  ...
  const decided = await transitionWithLog(db, {
    txid,
    fromState: "H_RESERVED (H-reserve funds are held) (H-reserve funds are held) (H-reserve funds are held) (H-reserve funds are held)",
...
/**
 * 名義不一致サスペンド後の再開: PRECHECKED_SUSPENDED → PRECHECKED → H_RESERVED (H-reserve funds are held) (H-reserve funds are held) (H-reserve funds are held) (H-reserve funds are held)
...
export async function resumeFromNameCheckSuspended(
  ...
  return { ok: true, state: "H_RESERVED (H-reserve funds are held) (H-reserve funds are held) (H-reserve funds are held) (H-reserve funds are held)" };
}
