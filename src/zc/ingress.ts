 ...
 * @module zc/ingress
 */
import type { Env, PaymentInitiatedRequest, HtlcCreateRequest, HtlcClaimRequest, GtidRegisterRequest, RtpRequestInput, TransferAuthorizeRequest, TransferCancelRequest } from '../types'
...
import {
  createAuthRequest, approveAuthRequest, declineAuthRequest,
...
export async function handlePostTransfers(req: Request, env: Env): Promise<Response> {
  ...
  if (participant?.tx_amount_limit != null && body.amount.value > participant.tx_amount_limit) {
    await completeIdempotency(idempKey, { result: 'REJECTED', reason_code: 'AMOUNT_EXCEEDS_TX_LIMIT' }, db)
    return jsonError(422, 'AMOUNT_EXCEEDS_TX_LIMIT', `amount ${body.amount.value} exceeds per-transaction limit ${participant.tx_amount_limit}`)
  }
  ...
  await db.prepare(
    ...
    body.txid, body.lane, body.amount.value,
    body.payer.bank_id, body.payer.account_hash,
    body.payee.bank_id, body.payee.account_hash ?? null,
    body.pspr_ref ?? null, body.purpose, idempKey,
    body.expires_at ?? null,
    isCrossBorder, fatfDataJson, fatf16Applicable,
    now, now,
  ).run()

  await writeFinalityLog(db, {
    txid: body.txid, event_type: 'PaymentInitiated', state_from: null, state_to: 'RECEIVED',
  ...
  switch (body.lane) {
    ...
    case 'DEFERRED':
      ...
      await env.QUEUE.send({ type: 'ZC_STATE_ADVANCE', payload: { txid: body.txid, action: 'ADVANCE_BULK' }, txid: body.txid, attempt: 0, enqueued_at: now })
      break
    case 'HIGH_VALUE':
      result = processHighValueIngress(body)
      await env.QUEUE.send({ type: 'ZC_STATE_ADVANCE', payload: { txid: body.txid, action: 'ADVANCE_HV' }, txid: body.txid, attempt: 0, enqueued_at: now })
      break
    case 'RTP':
...
// ---------------------------------------------------------------------------
export async function handlePostHtlcClaim(req: Request, htlcId: string, env: Env): Promise<Response> {
...
export async function handlePostRtpRequest(req: Request, env: Env): Promise<Response> {
  ...
  const result = await registerRtpRequest(
    ...
    { payeeName: body.payee_name, description: body.description, payeeAccountHash: body.payee_account },
    env,
  )
  const resp = { result: result.result === 'REGISTERED' ? 'INGRESS_ACCEPTED' : 'DUPLICATE', rtp_id: result.rtpId, state: 'REQUESTED' }
...
export async function handlePostCancel(req: Request, txid: string, env: Env): Promise<Response> {
  ...
  const now = nowISO()
  const tx = await db
    .prepare(`SELECT state, h_reservation_id, payer_bank_id, version FROM Transactions WHERE txid = ?`)
    .bind(txid)
    .first<{ state: string; h_reservation_id: string | null; payer_bank_id: string; version: number }>()
...
export async function handlePostParticipantRegister(req: Request, env: Env): Promise<Response> {
  const body = await parseBody<{ bank_id: string; bank_name: string; ingress_base_url: string; h_limit: number }>(req)
  if (!body) return jsonError(400, 'INVALID_JSON', 'invalid body')
  ...
  await env.DB.prepare(
    `INSERT OR REPLACE INTO Participants (bank_id, bank_name, ingress_base_url, h_limit, h_used, is_active, registered_at)
     VALUES (?, ?, ?, ?, 0, 1, ?)`
  ).bind(body.bank_id, body.bank_name, body.ingress_base_url, body.h_limit, now).run()
...
export async function handleSeed(env: Env): Promise<Response> {
  ...
  const deleteTargets = [
    'TxEventLog', 'BankAuditLog', 'PaymentApprovalRequests', 'PaymentFilters',
    'HtlcAuthRequests', 'HtlcAuthWhitelist', 'FinalityLog', 'IdempotencyKeys',
    'Cases', 'Vault', 'PsprRegistry', 'RtpRequests', 'RtpRequestRows', 'GtidLegs', 'GtidTransactions',
    'HtlcContracts', 'DnsNetPositions', 'DnsCycles', 'HReservations', 'Transactions',
    'Participants', 'ZcRequests', 'SuspenseDetails', 'DailyBalances', 'BankJournals',
    'BankAccounts', 'InterestRates',
    // migration 0003+ で追加されたテーブル
    'AccountVerifications', 'CreditNotifications', 'CrossBorderTransactions',
  ...
  return json(200, { result: 'RESET_AND_SEEDED', reset_at: now, initial_balance_per_account: 1000000 })
}
...
export async function handleAddBank(req: Request, env: Env): Promise<Response> {
  ...
  const maxBank = await db.prepare(
    `SELECT bank_id FROM Participants ORDER BY bank_id DESC LIMIT 1`
...
export async function handleDeleteBank(bankId: string, env: Env): Promise<Response> {
  ...
  ).bind(bankId, bankId).first<{ cnt: number }>()
...
export async function handleListBanks(env: Env): Promise<Response> {
  const rows = await env.DB.prepare(
    `SELECT * FROM Participants ORDER BY bank_id ASC`
  ).all()
...
export async function handleAccountNameLookup(accountId: string, env: Env): Promise<Response> {
  ...
  ).bind(accountId, bankCode).first<{ account_id: string; customer_name: string; bank_id: string; status: string; account_type: string }>()
  ...
  const bank = await env.DB.prepare(
    `SELECT bank_name FROM Participants WHERE bank_id=?`
  ).bind(bankCode).first<{ bank_name: string }>()
  return json(200, {
...
export async function handleSimSetup(req: Request, env: Env): Promise<Response> {
  ...
  try { params = await req.json() } catch { /* use defaults */ }
  ...
  const personalNames = ['田中太郎','鈴木花子','佐藤健','高橋美咲','伊藤誠','渡辺直子','山本浩二','中村愛','小林剛','加藤由美']
  ...
  for (const bankId of bankIds) {
    ...
    for (let j = 0; j < accountsPerBank; j++) {
      ...
      const nameBase    = isPersonal ? personalNames[j % personalNames.length] : corpNames[j % corpNames.length]
      ...
      stmts.push(db.prepare(
        `INSERT INTO BankJournals (journal_id, bank_id, account_id, amount, tx_type, tx_group_id, description, value_date, created_at)
         VALUES (?, ?, ?, ?, 'CASH', ?, ?, ?, ?)`
      ).bind(`JNL-${newUUID()}`, bankId, accountId, deposit, txGroupId, '一括開設初期入金', today, now))
      stmts.push(db.prepare(
        `INSERT INTO BankJournals (journal_id, bank_id, account_id, amount, tx_type, tx_group_id, description, value_date, created_at)
         VALUES (?, ?, ?, ?, 'CASH', ?, ?, ?, ?)`
      ).bind(`JNL-${newUUID()}`, bankId, cashAcct, -deposit, txGroupId, '一括開設 現金offset', today, now))
...
export async function handleSimSetupOneBank(req: Request, env: Env): Promise<Response> {
  ...
  const BANK_NAMES: Record<string, string> = {
    ...
    '009': '淡路銀行', '010': '阿波銀行', '011': '紀伊銀行', '012': '相模銀行',
    '013': '駿河銀行', '014': '甲斐銀行', '015': '信濃銀行', '016': '近江銀行',
    '017': '丹波銀行', '018': '但馬銀行', '019': '播磨銀行', '020': '美作銀行',
  }
  ...
  for (let j = 0; j < accountsPerBank; j++) {
    ...
    stmts.push(db.prepare(
      `INSERT INTO BankJournals (journal_id, bank_id, account_id, amount, tx_type, tx_group_id, description, value_date, created_at)
       VALUES (?, ?, ?, ?, 'CASH', ?, ?, ?, ?)`
    ).bind(`JNL-${newUUID()}`, nextCode, accountId, deposit, txGroupId, '一括開設初期入金', today, now))
    stmts.push(db.prepare(
      `INSERT INTO BankJournals (journal_id, bank_id, account_id, amount, tx_type, tx_group_id, description, value_date, created_at)
       VALUES (?, ?, ?, ?, 'CASH', ?, ?, ?, ?)`
    ).bind(`JNL-${newUUID()}`, nextCode, cashAcct, -deposit, txGroupId, '一括開設 現金offset', today, now))
  }
  ...
  ).bind(nextCode).first<{ cnt: number }>()
...
export async function handleHtlcAuthRequest(req: Request, env: Env): Promise<Response> {
  ...
  if (!body) return jsonError(400, 'INVALID_JSON', 'invalid body')
  if (!body.auth_id || !body.payee_bank_id || !body.payee_account_hash
    || !body.payer_bank_id || !body.payer_account_hash || !body.amount
    || !body.auth_expires_at || !body.capture_expires_at || !body.idempotency_key) {
    return jsonError(400, 'MISSING_FIELDS', 'auth_id, payee/payer info, amount, expires_at, idempotency_key required')
  }
...
export async function handleHtlcAuthApprove(req: Request, authId: string, env: Env): Promise<Response> {
...
export async function handleHtlcAuthDecline(req: Request, authId: string, env: Env): Promise<Response> {
  const body = await parseBody<{ reason?: string; idempotency_key: string }>(req)
  if (!body?.idempotency_key) return jsonError(400, 'MISSING_FIELDS', 'idempotency_key required')
  const result = await declineAuthRequest(authId, { reason: body.reason, idempotency_key: body.idempotency_key }, env)
  if (result.result === 'ERROR') return jsonError(400, result.reason_code!, result.reason_code!)
  return json(200, result)
}
...
export async function handleHtlcVoid(req: Request, htlcId: string, env: Env): Promise<Response> {
  const body = await parseBody<{ reason?: string; idempotency_key: string }>(req)
  if (!body?.idempotency_key) return jsonError(400, 'MISSING_FIELDS', 'idempotency_key required')
  const result = await voidHtlcAuth(htlcId, { reason: body.reason, idempotency_key: body.idempotency_key }, env)
  if (result.result === 'ERROR') return jsonError(400, result.reason_code!, result.reason_code!)
  return json(200, result)
}
