...
// ---------------------------------------------------------------------------
export {
  isValidTransition, ALLOWED_TRANSITIONS,
  writeFinalityLog, finalizeCancelledTx, suspendTx,
  checkAndFinalizeGtid,
  callBankReserveFunds, callBankExecuteDebit, callBankExecuteCredit,
  callBankReleaseReserve, callBankLegReadyCheck, callBankAuthorityCheck, callBankNameCheck,
}
...
export async function onPayeeExecConfirmed(
  ...
  await writeFinalityLog(db, {
    ...
    payload_json: JSON.stringify({ notification_id: notificationId, delivered: notificationDelivered, payee_bank_id: tx.payee_bank_id }),
    txid_or_gtid: txid,
  })

  try {
    const { handleBankIngress } = await import('../bank/ingress')
    await handleBankIngress(tx.payer_bank_id, 'debit-settled', {
      request_id: `DEBIT-SETTLED-${txid}`,
      txid, amount: { value: tx.amount_value, currency: 'JPY' },
      payee_bank_id: tx.payee_bank_id, settled_at: now,
    }, env)
  } catch (err) {
        ...
        if (bankResp.result === 'OK') {
          ...
          if (p.lane === 'HIGH_VALUE') {
            ...
            await initiateIgsSettlement(
              ...
              p.payer_bank_id, p.payee_bank_id, env,
            )
          }
        } else {
        ...
        if (bankResp.result === 'OK') {
          ...
          await writeFinalityLog(env.DB, {
            ...
            payload_json: JSON.stringify({
              reason_code: resumeFailReason, bank_id: p.payee_bank_id,
              bank_result: (bankResp as unknown as Record<string, unknown>).result,
              bank_reason_code: (bankResp as unknown as Record<string, unknown>).reason_code,
            }),
            txid_or_gtid: p.txid,
