...
const EVENT_RENDER: Record<string, EventRender> = {
  ...
  PayerExecConfirmed: {
    arrows: () => [{ kind: 'msg', from: 'PayerBank', to: 'ZC', text: '出金完了 (proof a)', dashed: true }],
    blurb: '送金元銀行の出金確認を取得',
  },
  PayeeExecConfirmed: {
    arrows: () => [{ kind: 'msg', from: 'PayeeBank', to: 'ZC', text: '入金完了 (proof b)', dashed: true }],
    blurb: '受取銀行の入金確認を取得',
  },
  Settled: {
  ...
  FilterRejected: {
    arrows: () => [{ kind: 'msg', from: 'PayeeBank', to: 'ZC', text: 'filter REJECT', dashed: true }],
...
function stateToVerb(state: string): string {
  switch (state) {
    case 'SETTLED':           return '最終確定済み'
    case 'CANCELLED':         return 'キャンセル済み'
    case 'FAILED_EXECUTION':  return '失敗'
    case 'SUSPENDED':         return '保留中'
    case 'H_RESERVED':        return 'H 予約済み'
    case 'DECIDED_TO_SETTLE': return '確定判断後の銀行処理中'
    case 'PAYER_EXEC_CONFIRMED': return '出金確認済み・入金待ち'
    case 'PAYEE_EXEC_CONFIRMED': return '入金確認済み・最終化待ち'
    case 'HTLC_LOCKED':       return 'HTLC ロック中'
    case 'HTLC_FULFILL_REQUESTED': return 'HTLC 解錠要求中'
    default:                  return `${state} 状態`
  }
}
