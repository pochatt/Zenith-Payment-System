 ...
 * @module zc/query
 */
import type { Env, QueryResponse, TransactionRow, GtidTransactionRow, HtlcContractRow, TxState, CaseState } from '../types'
...
export async function handleGetTransaction(txid: string, env: Env): Promise<Response> {
  ...
  const caseInfo = tx.case_id
    ? await db.prepare(`SELECT case_id, state FROM Cases WHERE case_id = ?`).bind(tx.case_id).first<{ case_id: string; state: CaseState }>()
    : null

  const decisionStatus: QueryResponse['decision']['status'] =
    tx.state === 'DECIDED_TO_SETTLE' || ['PAYER_EXEC_CONFIRMED', 'PAYEE_EXEC_CONFIRMED', 'SETTLED'].includes(tx.state)
      ? 'DECIDED_TO_SETTLE'
      : tx.state === 'DECIDED_CANCEL' || tx.state === 'CANCELLED'
      ? 'DECIDED_CANCEL'
      : 'NONE'

  const execA: 'NONE' | 'OK' | 'NG' =
    tx.payer_bank_proof_ref ? 'OK'
    : tx.state === 'FAILED_EXECUTION' ? 'NG'
    : 'NONE'

  const execB: 'NONE' | 'OK' | 'NG' =
    tx.payee_bank_proof_ref ? 'OK'
    : tx.state === 'FAILED_EXECUTION' ? 'NG'
    : 'NONE'

  const nextHint: QueryResponse['next_action_hint'] =
    tx.state === 'SETTLED' || tx.state === 'CANCELLED' ? 'WAIT'
    : tx.state === 'SUSPENDED' || tx.state === 'FAILED_EXECUTION' ? 'OPEN_CASE'
...
export async function handleGetDnsStatus(businessDate: string, env: Env): Promise<Response> {
  ...
  return json(200, { state: cycle.state, igs_mode: cycle.igs_mode, cycle_id: cycle.cycle_id, business_date: businessDate })
}
...
export async function handleListTransactions(req: Request, env: Env): Promise<Response> {
  ...
  if (txid)     { conds.push(`txid LIKE ?`);                        params.push(`${txid}%`) }
  if (account)  { conds.push(`(payer_account_hash = ? OR payee_account_hash = ?)`); params.push(account, account) }
  if (bankId)   { conds.push(`(payer_bank_id = ? OR payee_bank_id = ?)`);           params.push(bankId, bankId) }
  ...
  const countRow = await env.DB.prepare(countQuery).bind(...params).first<{count: number}>()
  ...
  return json(200, { transactions: rows.results, count: rows.results.length, total_count: totalCount })
}
...
export async function handleListHtlcs(req: Request, env: Env): Promise<Response> {
  ...
  const countRow = await env.DB.prepare(`SELECT COUNT(*) as count FROM HtlcContracts`).first<{count: number}>()
  const totalCount = countRow?.count ?? 0
...
export async function handleListGtids(req: Request, env: Env): Promise<Response> {
  ...
  const countRow = await env.DB.prepare(`SELECT COUNT(*) as count FROM GtidTransactions`).first<{count: number}>()
  const totalCount = countRow?.count ?? 0

  const rows = await env.DB.prepare(
    `SELECT * FROM GtidTransactions ORDER BY created_at DESC LIMIT ? OFFSET ?`
  ).bind(limit, offset).all<GtidTransactionRow>()
