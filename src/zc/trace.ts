...
export async function tracedBankCall<T extends { result: string }>(
  ...
  const duration_ms = Date.now() - t0
  const isOk = result.result === 'OK' || result.result === 'RESERVED'
    || result.result === 'RELEASED' || result.result === 'MATCH'
  await logTxEvent(db, {
    txid: params.txid,
    actor: `BANK_${params.bank_id}`,
    action: params.action,
    status: isOk ? 'OK' : 'NG',
    reason_code: isOk ? null : ((result as Record<string, unknown>).reason_code as string | null ?? null),
    amount: params.amount,
    bank_id: params.bank_id,
