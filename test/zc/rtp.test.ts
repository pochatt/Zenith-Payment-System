...
function seedAccounts(db: MockD1Database) {
  for (const [bankId, accountId] of [[PAYER_BANK, PAYER_ACCOUNT], [PAYEE_BANK, PAYEE_ACCOUNT]]) {
    db.prepare(
...
describe('registerRtpRequest — REGISTERED vs DUPLICATE', () => {
  it('returns REGISTERED on first call', async () => {
    const result = await registerRtpRequest(
      d1 as any, 'RTP-001', PAYEE_BANK, PAYER_BANK,
      { value: 50_000, currency: 'JPY' }, FUTURE_EXPIRY, 'IK-RTP-001',
      { payeeName: 'Test Payee', description: 'Invoice #42' }, makeEnv(d1),
    )
  ...
  it('returns DUPLICATE on second call with same rtp_id', async () => {
    await registerRtpRequest(
      d1 as any, 'RTP-DUP-001', PAYEE_BANK, PAYER_BANK,
      { value: 10_000, currency: 'JPY' }, FUTURE_EXPIRY, 'IK-DUP-001',
      {}, makeEnv(d1),
    )
  ...
  it('creates RtpRequests row with REQUESTED state', async () => {
    await registerRtpRequest(
      d1 as any, 'RTP-ROW-001', PAYEE_BANK, PAYER_BANK,
      { value: 30_000, currency: 'JPY' }, FUTURE_EXPIRY, 'IK-ROW-001',
      {}, makeEnv(d1),
    )
    const row = await d1.prepare(`SELECT state, rtp_status FROM RtpRequests WHERE rtp_id=?`)
      .bind('RTP-ROW-001').first<{ state: string; rtp_status: string }>()
    expect(row?.state).toBe('REQUESTED')
  })

  it('updates rtp_status to NOTIFIED after bank notification', async () => {
    await registerRtpRequest(
      d1 as any, 'RTP-NOTIF-001', PAYEE_BANK, PAYER_BANK,
      { value: 20_000, currency: 'JPY' }, FUTURE_EXPIRY, 'IK-NOTIF-001',
      {}, makeEnv(d1),
    )
    const row = await d1.prepare(`SELECT rtp_status FROM RtpRequests WHERE rtp_id=?`)
      .bind('RTP-NOTIF-001').first<{ rtp_status: string }>()
    expect(row?.rtp_status).toBe('NOTIFIED')
  })

  it('writes RtpRequested FinalityLog entry', async () => {
    await registerRtpRequest(
      d1 as any, 'RTP-LOG-001', PAYEE_BANK, PAYER_BANK,
      { value: 15_000, currency: 'JPY' }, FUTURE_EXPIRY, 'IK-LOG-001',
      {}, makeEnv(d1),
    )
    const log = await d1.prepare(
      `SELECT event_type FROM FinalityLog WHERE event_type='RtpRequested' LIMIT 1`
  ...
  it('stores optional fields (payeeName, description, ediRef, payeeAccountHash)', async () => {
    await registerRtpRequest(
      d1 as any, 'RTP-OPT-001', PAYEE_BANK, PAYER_BANK,
      { value: 5_000, currency: 'JPY' }, FUTURE_EXPIRY, 'IK-OPT-001',
      {
        payeeName: 'Acme Corp',
    ...
    ).bind('RTP-OPT-001').first<{
      payee_name: string | null; description: string | null
      edi_ref: string | null; payee_account_hash: string | null
    }>()
...
describe('attemptRtp', () => {
  async function registerRtp(rtpId: string) {
    ...
    ).bind(rtpId, PAYEE_BANK, PAYER_BANK, FUTURE_EXPIRY)._runSync()
  }
  ...
  it('returns true and transitions to ATTEMPTED', async () => {
    ...
    const row = await d1.prepare(`SELECT state, linked_txid, attempt_count FROM RtpRequests WHERE rtp_id=?`)
      .bind('RTP-ATT-001').first<{ state: string; linked_txid: string; attempt_count: number }>()
  ...
  it('returns false and marks EXPIRED when RTP has passed expires_at', async () => {
    ...
    ).bind('RTP-EXP-001', PAYEE_BANK, PAYER_BANK, PAST_EXPIRY)._runSync()

    const ok = await attemptRtp('RTP-EXP-001', 'TX-EXP-LINK', makeEnv(d1))
    expect(ok).toBe(false)

    const row = await d1.prepare(`SELECT state FROM RtpRequests WHERE rtp_id=?`)
      .bind('RTP-EXP-001').first<{ state: string }>()
  ...
  it('returns false and marks FAILED when max_attempts reached', async () => {
    ...
    ).bind('RTP-MAX-001', PAYEE_BANK, PAYER_BANK, FUTURE_EXPIRY)._runSync()

    const ok = await attemptRtp('RTP-MAX-001', 'TX-MAX-LINK', makeEnv(d1))
    expect(ok).toBe(false)

    const row = await d1.prepare(`SELECT state FROM RtpRequests WHERE rtp_id=?`)
      .bind('RTP-MAX-001').first<{ state: string }>()
  ...
  it('returns false when RTP is already in ATTEMPTED state', async () => {
    ...
    ).bind('RTP-ATT-DUP-001', PAYEE_BANK, PAYER_BANK, FUTURE_EXPIRY)._runSync()
...
describe('respondToRtp', () => {
  async function registerRtp(rtpId: string, expiresAt = FUTURE_EXPIRY) {
    await registerRtpRequest(
      d1 as any, rtpId, PAYEE_BANK, PAYER_BANK,
      { value: 50_000, currency: 'JPY' }, expiresAt, `IK-${rtpId}`,
      { payeeAccountHash: PAYEE_ACCOUNT }, makeEnv(d1),
    )
  }

  it('ACCEPTED: creates a linked Transaction and returns txid', async () => {
    await registerRtp('RTP-RESP-ACC-001')
    const result = await respondToRtp(d1 as any, 'RTP-RESP-ACC-001', {
      response: 'ACCEPTED',
    ...
    const tx = await d1.prepare(`SELECT lane, state, amount_value FROM Transactions WHERE txid=?`)
      .bind(result.txid!).first<{ lane: string; state: string; amount_value: number }>()
  ...
  it('ACCEPTED: transitions RTP to TX_CREATED state', async () => {
    ...
    const result = await respondToRtp(d1 as any, 'RTP-RESP-ACC-002', {
      response: 'ACCEPTED',
      payer_account_id: PAYER_ACCOUNT,
    ...
    const row = await d1.prepare(`SELECT rtp_status, state FROM RtpRequests WHERE rtp_id=?`)
      .bind('RTP-RESP-ACC-002').first<{ rtp_status: string; state: string }>()
  ...
  it('ACCEPTED: writes RtpAccepted FinalityLog entry', async () => {
    ...
    await respondToRtp(d1 as any, 'RTP-RESP-ACC-003', {
      response: 'ACCEPTED',
      payer_account_id: PAYER_ACCOUNT,
    ...
    const log = await d1.prepare(
      `SELECT event_type FROM FinalityLog WHERE event_type='RtpAccepted' LIMIT 1`
    ).first<{ event_type: string }>()
    expect(log).not.toBeNull()
  })
  ...
  it('REJECTED: returns DECLINED and sets rtp_status to DECLINED', async () => {
    ...
    const result = await respondToRtp(d1 as any, 'RTP-RESP-REJ-001', {
      response: 'REJECTED',
      payer_account_id: PAYER_ACCOUNT,
    ...
    const row = await d1.prepare(`SELECT rtp_status, state FROM RtpRequests WHERE rtp_id=?`)
      .bind('RTP-RESP-REJ-001').first<{ rtp_status: string; state: string }>()
    expect(row?.rtp_status).toBe('DECLINED')
    expect(row?.state).toBe('FAILED')
  })

  it('returns NOT_FOUND for unknown rtp_id', async () => {
    const result = await respondToRtp(d1 as any, 'RTP-UNKNOWN', {
      response: 'ACCEPTED',
      payer_account_id: PAYER_ACCOUNT,
  ...
  it('returns ALREADY_RESPONDED when RTP was previously accepted', async () => {
    ...
    await respondToRtp(d1 as any, 'RTP-RESP-DUP-001', {
      response: 'ACCEPTED',
      payer_account_id: PAYER_ACCOUNT,
      idempotency_key: 'IK-RESP-DUP-001',
    }, makeEnv(d1))

    const second = await respondToRtp(d1 as any, 'RTP-RESP-DUP-001', {
      response: 'ACCEPTED',
      payer_account_id: PAYER_ACCOUNT,
  ...
  it('returns EXPIRED when RTP expires_at has passed', async () => {
    ...
    const result = await respondToRtp(d1 as any, 'RTP-RESP-EXP-001', {
      response: 'ACCEPTED',
      payer_account_id: PAYER_ACCOUNT,
...
describe('settleRtp', () => {
  it('transitions RTP to SETTLED state', async () => {
    ...
    ).bind(PAYEE_BANK, PAYER_BANK, FUTURE_EXPIRY)._runSync()

    await settleRtp('RTP-SETTLE-001', d1 as any)

    const row = await d1.prepare(`SELECT state FROM RtpRequests WHERE rtp_id=?`)
      .bind('RTP-SETTLE-001').first<{ state: string }>()
...
describe('expireRtpRequests', () => {
  it('expires CREATED/NOTIFIED RTPs past their expires_at', async () => {
    ...
    for (const rtpId of ['RTP-EXP-CRON-001', 'RTP-EXP-CRON-002']) {
      ...
      ).bind(rtpId, PAYEE_BANK, PAYER_BANK, PAST_EXPIRY)._runSync()
    }
    // One still-valid RTP
    d1.prepare(
      `INSERT INTO RtpRequests
    ...
    for (const rtpId of ['RTP-EXP-CRON-001', 'RTP-EXP-CRON-002']) {
      const row = await d1.prepare(`SELECT state, rtp_status FROM RtpRequests WHERE rtp_id=?`)
        .bind(rtpId).first<{ state: string; rtp_status: string }>()
      expect(row?.state).toBe('EXPIRED')
      expect(row?.rtp_status).toBe('EXPIRED')
    }

    const valid = await d1.prepare(`SELECT state FROM RtpRequests WHERE rtp_id='RTP-VALID-001'`)
      .first<{ state: string }>()
...
describe('getRtpStatus', () => {
  ...
  it('returns EXPIRED status for EXPIRED rtp_status', async () => {
    ...
    ).bind(PAYEE_BANK, PAYER_BANK, PAST_EXPIRY)._runSync()
