  ...
  return db.prepare(
    `INSERT INTO Transactions
     ...
     VALUES (?, 'EXPRESS', ?, 1000, 'JPY', '001', '0010000001', '002', '0020000001',
             ?, '1.0', 0, ?, ?)`
  ).bind(txid, state, `idem-${txid}`, now, now).run()
}
...
describe('transitionWithLog — state machine enforcement', () => {
  ...
  it('rejects a transition not present in ALLOWED_TRANSITIONS (default mode)', async () => {
    ...
    const row = await db.prepare(`SELECT state FROM Transactions WHERE txid = ?`)
      .bind('TX-ILL-001').first<{ state: string }>()
    expect(row?.state).toBe('RECEIVED')
    const flCount = await db.prepare(`SELECT COUNT(*) AS c FROM FinalityLog WHERE txid = ?`)
      .bind('TX-ILL-001').first<{ c: number }>()
...
describe('transitionWithLog — atomic CAS + FinalityLog batch', () => {
  ...
  it('writes the state advance AND the FinalityLog entry together', async () => {
    ...
    const tx = await db.prepare(`SELECT state, version FROM Transactions WHERE txid = ?`)
      .bind('TX-ATOM-001').first<{ state: string; version: number }>()
    ...
    ).bind('TX-ATOM-001').all<{
      state_from: string; state_to: string; event_type: string;
      prev_hash: string; entry_hash: string; event_seq: number
    }>()
  ...
  it('losing CAS produces neither a state change nor a FinalityLog row', async () => {
    ...
    const p1 = transitionWithLog(db as any, {
      txid: 'TX-ATOM-002', fromState: 'RECEIVED', toState: 'PRECHECKED',
      eventType: 'PreCheckPassed',
    })
    ...
    ).bind('TX-ATOM-002').first<{ c: number }>()
    expect(fl?.c).toBe(1)
  })
})
...
describe('event_seq — monotonic FinalitySeq counter', () => {
  ...
  it('assigns strictly increasing event_seq to consecutive writes', async () => {
    await writeFinalityLog(db as any, {
      txid: 'TX-SEQ-001', event_type: 'PaymentInitiated',
      state_from: null, state_to: 'RECEIVED',
      payload_json: '{}', txid_or_gtid: 'TX-SEQ-001',
    })
    await writeFinalityLog(db as any, {
      txid: 'TX-SEQ-001', event_type: 'PreCheckPassed',
      state_from: 'RECEIVED', state_to: 'PRECHECKED',
      payload_json: '{}', txid_or_gtid: 'TX-SEQ-001',
    })
    await writeFinalityLog(db as any, {
      txid: 'TX-SEQ-002', event_type: 'PaymentInitiated',
      state_from: null, state_to: 'RECEIVED',
      payload_json: '{}', txid_or_gtid: 'TX-SEQ-002',
    })

    const rows = await db.prepare(
      `SELECT event_seq FROM FinalityLog ORDER BY event_seq ASC`
  ...
  it('event_seq is UNIQUE across all FinalityLog rows', async () => {
    ...
    for (let i = 0; i < 50; i++) {
      await writeFinalityLog(db as any, {
        txid: `TX-SEQ-MULTI-${i}`, event_type: 'PaymentInitiated',
        state_from: null, state_to: 'RECEIVED',
        payload_json: `{"i":${i}}`, txid_or_gtid: `TX-SEQ-MULTI-${i}`,
      })
    }
    const dup = await db.prepare(
      `SELECT event_seq, COUNT(*) AS c FROM FinalityLog GROUP BY event_seq HAVING c > 1`
  ...
  it('prepareFinalityLogRow allocates seqs from the same shared counter', async () => {
    ...
    const r1 = await prepareFinalityLogRow(db as any, {
      txid: 'TX-SEQ-X', event_type: 'PaymentInitiated',
      state_from: null, state_to: 'RECEIVED',
      payload_json: '{}', txid_or_gtid: 'TX-SEQ-X',
    })
    const r2 = await prepareFinalityLogRow(db as any, {
      txid: 'TX-SEQ-Y', event_type: 'PaymentInitiated',
      state_from: null, state_to: 'RECEIVED',
      payload_json: '{}', txid_or_gtid: 'TX-SEQ-Y',
    })
...
describe('cancelInFlightTx — batched DecidedCancel + sideUpdates', () => {
  ...
  it('records the DecidedCancel FinalityLog entry atomically with the CAS', async () => {
    ...
    const tx = await db.prepare(`SELECT state, reason_code FROM Transactions WHERE txid = ?`)
      .bind('TX-CANC-001').first<{ state: string; reason_code: string }>()
    ...
    ).bind('TX-CANC-001').all<{ event_type: string; state_from: string; state_to: string }>()
  ...
  it('losing the CAS does not write a phantom DecidedCancel entry', async () => {
    ...
    const ok1 = await cancelInFlightTx(db as any, {
      txid: 'TX-CANC-002', reasonCode: 'FIRST', fromStates: ['PRECHECKED'],
    })
    ...
    const ok2 = await cancelInFlightTx(db as any, {
      txid: 'TX-CANC-002', reasonCode: 'SECOND', fromStates: ['PRECHECKED'],
    })
    expect(ok2).toBe(false)

    const decidedCancels = await db.prepare(
      `SELECT COUNT(*) AS c FROM FinalityLog
       WHERE txid = ? AND event_type = 'DecidedCancel'`
    ).bind('TX-CANC-002').first<{ c: number }>()
...
describe('suspendTx — GTID cascade via GtidLegs, not txid prefix', () => {
  ...
  it('finds the GTID via GtidLegs.txid when the leg txid does not use the legacy TX-GT- prefix', async () => {
    ...
    ).bind(legTxid, now, now).run()

    await seedTx(db, legTxid, 'PAYER_EXEC_CONFIRMED')
    // Move it into a state from which SUSPENDED is valid.
    await db.prepare(`UPDATE Transactions SET state='PAYER_EXEC_CONFIRMED' WHERE txid=?`)
      .bind(legTxid).run()
    ...
    const tx = await db.prepare(`SELECT state FROM Transactions WHERE txid = ?`)
      .bind(legTxid).first<{ state: string }>()
