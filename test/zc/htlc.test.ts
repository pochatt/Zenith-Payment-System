...
function insertHtlcReceived(db: MockD1Database, htlcId: string, amount = 100_000, timelockISO = '2099-12-31T00:00:00Z') {
  ...
  ).bind(txid, amount, PAYER_BANK, PAYEE_BANK, `IK-${htlcId}`)._runSync()
  ...
  ).bind(htlcId, txid, hashlock, timelockISO, amount, PAYER_BANK, PAYEE_BANK)._runSync()
...
describe('lockHtlc', () => {
  it('transitions HtlcContracts and Transactions to HTLC_LOCKED', async () => {
    ...
    const htlc = await d1.prepare(`SELECT state FROM HtlcContracts WHERE htlc_id = ?`)
      .bind(htlcId).first<{ state: string }>()
    expect(htlc?.state).toBe('HTLC_LOCKED')

    const tx = await d1.prepare(`SELECT state FROM Transactions WHERE txid = ?`)
      .bind(`TX-HTLC-${htlcId}`).first<{ state: string }>()
...
describe('cancelHtlc — TOCTOU regression (Bug #1)', () => {
  ...
  it('transitions HTLC contract to DECIDED_CANCEL and writes FinalityLog', async () => {
    ...
    const log = await d1.prepare(
      `SELECT event_type FROM FinalityLog WHERE txid = ? AND event_type = 'HtlcCancelled'`
    ).bind(txid).first<{ event_type: string }>()
