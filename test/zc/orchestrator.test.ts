...
describe('writeFinalityLog', () => {
  it('writes a log entry that can be read back', async () => {
    await writeFinalityLog(d1 as any, {
      txid: 'TX-TEST-001', event_type: 'DecidedToSettle',
      state_from: 'H_RESERVED', state_to: 'DECIDED_TO_SETTLE',
      payload_json: '{"test":true}', txid_or_gtid: 'TX-TEST-001',
    })

    const row = await d1.prepare(
      `SELECT * FROM FinalityLog WHERE txid = ? ORDER BY event_seq DESC LIMIT 1`
    ).bind('TX-TEST-001').first<{ event_type: string; state_to: string; event_seq: number }>()
  ...
  it('produces strictly increasing event_seq for sequential writes', async () => {
    ...
    await writeFinalityLog(d1 as any, {
      txid: 'TX-SEQ-001', event_type: 'HReserved',
      state_from: 'PRECHECKED', state_to: 'H_RESERVED',
      payload_json: '{}', txid_or_gtid: 'TX-SEQ-001',
    })
    await writeFinalityLog(d1 as any, {
      txid: 'TX-SEQ-001', event_type: 'DecidedToSettle',
      state_from: 'H_RESERVED', state_to: 'DECIDED_TO_SETTLE',
      payload_json: '{}', txid_or_gtid: 'TX-SEQ-001',
    })

    const rows = await d1.prepare(
      `SELECT event_seq FROM FinalityLog WHERE txid = ? ORDER BY event_seq ASC`
    ).bind('TX-SEQ-001').all<{ event_seq: number }>()
  ...
  it('sets gtid column for GTID-prefixed txid_or_gtid', async () => {
    await writeFinalityLog(d1 as any, {
      txid: null, event_type: 'GtidDecided',
      state_from: 'GT_PRECHECKED', state_to: 'GT_DECIDED_TO_SETTLE',
      payload_json: '{}', txid_or_gtid: 'GTID-TEST-001',
    })

    const row = await d1.prepare(
      `SELECT gtid, txid FROM FinalityLog WHERE event_type = 'GtidDecided' LIMIT 1`
...
describe('finalizeCancelledTx', () => {
  ...
  it('transitions DECIDED_CANCEL → CANCELLED and writes FinalityLog', async () => {
    ...
    const row = await d1.prepare(
      `SELECT state FROM Transactions WHERE txid = ?`
    ).bind('TX-CANCEL-001').first<{ state: string }>()
    expect(row?.state).toBe('CANCELLED')

    const log = await d1.prepare(
      `SELECT event_type FROM FinalityLog WHERE txid = ? AND event_type = 'Cancelled'`
    ).bind('TX-CANCEL-001').first<{ event_type: string }>()
  ...
  it('is idempotent: calling twice does not throw and CANCELLED stays CANCELLED', async () => {
    ...
    const row = await d1.prepare(
      `SELECT state FROM Transactions WHERE txid = ?`
    ).bind('TX-CANCEL-002').first<{ state: string }>()
    expect(row?.state).toBe('CANCELLED')
  })
  ...
  it('does not transition a non-DECIDED_CANCEL state', async () => {
    ...
    const row = await d1.prepare(
      `SELECT state FROM Transactions WHERE txid = ?`
    ).bind('TX-CANCEL-003').first<{ state: string }>()
...
describe('suspendTx', () => {
  function insertTx(txid: string, state: string, version = 0) {
    ...
    ).bind(txid, state, `IK-${txid}`, version)._runSync()
  }
  ...
  it('transitions DECIDED_TO_SETTLE → SUSPENDED and writes FinalityLog', async () => {
    ...
    ).bind('TX-SUSP-001').first<{ state: string; reason_code: string | null }>()
    expect(row?.state).toBe('SUSPENDED')
    expect(row?.reason_code).toBe('EXEC_DEBIT_FAILED')

    const log = await d1.prepare(
      `SELECT event_type FROM FinalityLog WHERE txid = ? AND event_type = 'Suspended'`
    ).bind('TX-SUSP-001').first<{ event_type: string }>()
    expect(log).not.toBeNull()
  })
  ...
  it('does not suspend a SETTLED transaction (invalid transition)', async () => {
    ...
    const row = await d1.prepare(
      `SELECT state FROM Transactions WHERE txid = ?`
    ).bind('TX-SUSP-002').first<{ state: string }>()
