...
function seedHighValueTx(txid: string, amount = 200_000_000) {
  ...
  ).bind(txid, amount, PAYER_BANK, PAYER_ACCOUNT, PAYEE_BANK, PAYEE_ACCOUNT, `IK-${txid}`)._runSync()
}
...
describe('HIGH_VALUE lane — H-model bypass (spec §HV)', () => {
  ...
  it('creates NO HReservations row (H is never reserved for HIGH_VALUE)', async () => {
    ...
    const hRes = await d1.prepare(
      `SELECT reservation_id FROM HReservations WHERE txid=?`,
    ).bind('TX-HV-HBYP-002').first<{ reservation_id: string }>()
  ...
  it('advances to DECIDED_TO_SETTLE using BOJ pre-fund check', async () => {
    ...
    const tx = await d1.prepare(`SELECT state FROM Transactions WHERE txid=?`)
      .bind('TX-HV-HBYP-003').first<{ state: string }>()
...
describe('HIGH_VALUE lane — BOJ pre-fund check', () => {
  it('rejects with BOJ_INSUFFICIENT_FUNDS when payer has no BOJ pre-fund', async () => {
    ...
    d1.prepare(`DELETE FROM BankJournals WHERE account_id=?`)
      .bind(`${PAYER_BANK}-BOJ`)._runSync()
    ...
    const tx = await d1.prepare(`SELECT state, reason_code FROM Transactions WHERE txid=?`)
      .bind('TX-HV-NOFUND-001').first<{ state: string; reason_code: string | null }>()
  ...
  it('rejects when transfer amount exceeds remaining BOJ pre-fund', async () => {
    ...
    const tx = await d1.prepare(`SELECT state, reason_code FROM Transactions WHERE txid=?`)
      .bind('TX-HV-EXCEED-001').first<{ state: string; reason_code: string | null }>()
    expect(tx?.state).toBe('CANCELLED')
    expect(tx?.reason_code).toBe('BOJ_INSUFFICIENT_FUNDS')
  })
  ...
  it('accepts when transfer amount exactly equals BOJ pre-fund capacity', async () => {
    ...
    const tx = await d1.prepare(`SELECT state FROM Transactions WHERE txid=?`)
      .bind('TX-HV-EXACT-001').first<{ state: string }>()
    expect(tx?.state).toBe('DECIDED_TO_SETTLE')
  })
})
