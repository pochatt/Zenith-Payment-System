...
import {
  registerAuthWhitelist, createAuthRequest, approveAuthRequest, captureHtlcAuth,
...
function insertReceivedTx(db: MockD1Database, args: {
  txid: string
  lane: string
  amount: number
  payerBank: string
  ...
  db.prepare(
    ...
    args.txid, args.lane, args.amount,
    args.payerBank, args.payerAcc, args.payeeBank, args.payeeAcc,
    args.psprRef ?? null, args.idempotencyKey ?? `IK-${args.txid}`,
  )._runSync()
}
...
describe('EXPRESS — end-to-end balance', () => {
  it('credits payee exactly once and preserves per-bank zero-sum', async () => {
    ...
    insertReceivedTx(d1, {
      txid: 'TX-EXP-INT-001', lane: 'EXPRESS', amount,
      payerBank: BANK_A, payerAcc: ACC_A,
      payeeBank: BANK_B, payeeAcc: ACC_B,
    })

    const res = await processExpress({
      txid: 'TX-EXP-INT-001', lane: 'EXPRESS',
    ...
    const payeeCreditEntries = await d1.prepare(
      `SELECT COUNT(*) AS c FROM BankJournals
        WHERE account_id = ? AND txid = ? AND amount > 0`,
    ).bind(ACC_B, 'TX-EXP-INT-001').first<{ c: number }>()
  ...
  it('SETTLED txid leaves all balance deltas where DNS can still clear them', async () => {
    const env = makeEnv(d1)
    insertReceivedTx(d1, {
      txid: 'TX-EXP-INT-002', lane: 'EXPRESS', amount: 200_000,
      payerBank: BANK_A, payerAcc: ACC_A,
      payeeBank: BANK_B, payeeAcc: ACC_B,
    })
    await processExpress({
      txid: 'TX-EXP-INT-002', lane: 'EXPRESS',
    ...
    const tx = await d1.prepare(`SELECT state FROM Transactions WHERE txid=?`)
      .bind('TX-EXP-INT-002').first<{ state: string }>()
...
describe('STANDARD — end-to-end balance', () => {
  it('credits payee exactly +amount after authorize', async () => {
    ...
    insertReceivedTx(d1, {
      txid: 'TX-STD-INT-001', lane: 'STANDARD', amount,
      payerBank: BANK_A, payerAcc: ACC_A,
      payeeBank: BANK_B, payeeAcc: ACC_B,
    })
    ...
    const mid = await d1.prepare(`SELECT state FROM Transactions WHERE txid=?`)
      .bind('TX-STD-INT-001').first<{ state: string }>()
...
describe('HTLC — preimage release end-to-end', () => {
  it('credits payee +amount when preimage matches', async () => {
    ...
    await drain(env)
    const lockedHtlc = await d1.prepare(`SELECT state FROM HtlcContracts WHERE htlc_id=?`)
      .bind('HTLC-INT-001').first<{ state: string }>()
    expect(lockedHtlc?.state).toBe('HTLC_LOCKED')

    const claimed = await claimHtlc({
      htlc_id: 'HTLC-INT-001', preimage,
  ...
  it('does not credit when preimage is wrong (REJECTED)', async () => {
    ...
    const rejected = await claimHtlc({
      htlc_id: 'HTLC-INT-002', preimage: '00'.repeat(32),
...
describe('HTLC_AUTH — end-to-end balance', () => {
  it('payee receives +amount after capture (no funds leak to suspense)', async () => {
    ...
    await registerAuthWhitelist({
      payee_bank_id: BANK_B, payee_account_hash: ACC_B,
      max_amount: 100_000, description: 'integration test',
    } as any, d1 as any)
    ...
    const ar = await createAuthRequest({
      ...
      payer_bank_id: BANK_A, payer_account_hash: ACC_A,
      amount: { value: amount, currency: 'JPY' },
      auth_expires_at: farFuture, capture_expires_at: farFuture2,
    } as any, env as any)
    expect(ar.result).toBe('AUTH_REQUESTED')

    const apv = await approveAuthRequest('AUTH-INT-001', {
    ...
    const txAfterApprove = await d1.prepare(`SELECT state FROM Transactions WHERE txid=?`)
      .bind('TX-HAUTH-AUTH-INT-001').first<{ state: string }>()
    expect(txAfterApprove?.state).toBe('HTLC_LOCKED')

    const cap = await captureHtlcAuth('HAUTH-AUTH-INT-001', {
    ...
    const txFinal = await d1.prepare(`SELECT state FROM Transactions WHERE txid=?`)
      .bind('TX-HAUTH-AUTH-INT-001').first<{ state: string }>()
...
describe('BULK — DNS-settled balance', () => {
  it('DECIDED_TO_SETTLE before DNS, then DNS clears the suspense and reaches SETTLED', async () => {
    ...
    insertReceivedTx(d1, {
      txid: 'TX-BULK-INT-001', lane: 'BULK', amount,
      payerBank: BANK_A, payerAcc: ACC_A,
      payeeBank: BANK_B, payeeAcc: ACC_B,
    })
    ...
    const mid = await d1.prepare(`SELECT state FROM Transactions WHERE txid=?`)
      .bind('TX-BULK-INT-001').first<{ state: string }>()
    ...
    const finalState = await d1.prepare(`SELECT state FROM Transactions WHERE txid=?`)
      .bind('TX-BULK-INT-001').first<{ state: string }>()
...
describe('HIGH_VALUE — RTGS end-to-end balance', () => {
  it('settles via IGS, moving Customer→ZCS→BOJ and reaching SETTLED', async () => {
    ...
    insertReceivedTx(d1, {
      txid: 'TX-HV-INT-001', lane: 'HIGH_VALUE', amount,
      payerBank: BANK_A, payerAcc: ACC_A,
      payeeBank: BANK_B, payeeAcc: ACC_B,
    })
    ...
    const final = await d1.prepare(`SELECT state FROM Transactions WHERE txid=?`)
      .bind('TX-HV-INT-001').first<{ state: string }>()
...
describe('GTID — 1×1 atomic transfer', () => {
  it('credits payee +amount after PAYER leg settles via DNS', async () => {
    ...
    await registerGtid({
      ...
      legs: [
        { leg_id: 'GT-INT-001-A', role: 'PAYER',
          bank_id: BANK_A, account_hash: ACC_A,
          amount: { value: amount, currency: 'JPY' } },
        { leg_id: 'GT-INT-001-B', role: 'PAYEE',
          bank_id: BANK_B, account_hash: ACC_B,
          amount: { value: amount, currency: 'JPY' } },
      ],
    } as any, env as any)
    ...
    const tx = await d1.prepare(`SELECT state FROM Transactions WHERE txid=?`)
      .bind('TX-GT-GT-INT-001-A').first<{ state: string }>()
    expect(tx?.state).toBe('SETTLED')
    ...
    const gt = await d1.prepare(`SELECT state FROM GtidTransactions WHERE gtid=?`)
      .bind('GT-INT-001').first<{ state: string }>()
...
describe('GTID — 2×2 with PAYEE inserted in reverse leg_id order', () => {
  ...
  it('pairs PAYER A↔PAYEE A2 and PAYER B↔PAYEE B2 regardless of insertion order', async () => {
    ...
    await registerGtid({
      ...
      expires_at: '2099-12-31T00:00:00Z',
      legs: [
        { leg_id: 'GT-2X2-A', role: 'PAYER',
          bank_id: BANK_A, account_hash: ACC_A,
          amount: { value: amtSmall, currency: 'JPY' } },
        { leg_id: 'GT-2X2-B', role: 'PAYER',
          bank_id: BANK_A, account_hash: ACC_A2,
          amount: { value: amtLarge, currency: 'JPY' } },
        // Inserted PAYEE B (large) FIRST despite alphabetically later leg_id
        { leg_id: 'GT-2X2-Z-PAYEE-B', role: 'PAYEE',
          bank_id: BANK_B, account_hash: ACC_B2,
          amount: { value: amtLarge, currency: 'JPY' } },
        { leg_id: 'GT-2X2-A-PAYEE-A', role: 'PAYEE',
          bank_id: BANK_B, account_hash: ACC_B,
          amount: { value: amtSmall, currency: 'JPY' } },
      ],
    } as any, env as any)
...
describe('Multi-lane interleave', () => {
  it('Express + Standard + HTLC in the same DB all settle correctly', async () => {
    ...
    insertReceivedTx(d1, {
      txid: 'TX-MIX-EXP', lane: 'EXPRESS', amount: 11_000,
      payerBank: BANK_A, payerAcc: ACC_A,
      payeeBank: BANK_B, payeeAcc: ACC_B,
    })
    insertReceivedTx(d1, {
      txid: 'TX-MIX-STD', lane: 'STANDARD', amount: 22_000,
      payerBank: BANK_A, payerAcc: ACC_A2,
      payeeBank: BANK_B, payeeAcc: ACC_B2,
    })

    await processExpress({
      txid: 'TX-MIX-EXP', lane: 'EXPRESS',
    ...
    await drain(env)
    await claimHtlc({
      htlc_id: 'HTLC-MIX-002', preimage: h2.preimage!,
