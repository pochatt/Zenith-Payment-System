...
describe('validatePaymentInitiated', () => {
  ...
  it('rejects amount.value of 0', () => {
    const res = validatePaymentInitiated({ ...validPayment(), amount: { value: 0, currency: 'JPY' } })
    expect(res.ok).toBe(false)
    expect(res.reason_code).toBe('INVALID_AMOUNT')
  })

  it('rejects fractional amount', () => {
    const res = validatePaymentInitiated({ ...validPayment(), amount: { value: 100.5, currency: 'JPY' } })
    expect(res.ok).toBe(false)
    expect(res.reason_code).toBe('INVALID_AMOUNT')
  })

  it('rejects non-JPY currency', () => {
    const res = validatePaymentInitiated({ ...validPayment(), amount: { value: 100, currency: 'USD' as any } })
    expect(res.ok).toBe(false)
...
describe('validateGtidRegister', () => {
  const validLegs = [
    { leg_id: 'L1', role: 'PAYER', bank_id: '001', account_hash: '0010000001', amount: { value: 1000, currency: 'JPY' } },
    { leg_id: 'L2', role: 'PAYEE', bank_id: '002', account_hash: '0020000001', amount: { value: 1000, currency: 'JPY' } },
  ]

  it('passes with 2 valid legs', () => {
    expect(validateGtidRegister({ gtid: 'GT-001', legs: validLegs, idempotency_key: 'k' }).ok).toBe(true)
  })
  ...
  it('rejects fewer than 2 legs', () => {
    const res = validateGtidRegister({ gtid: 'GT-001', legs: [validLegs[0]], idempotency_key: 'k' })
    expect(res.ok).toBe(false)
