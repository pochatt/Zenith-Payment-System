...
- ~~**ハッシュチェーン検証 cron**: `verifyChain` は API 経由でのみ走る。
  日次自動化が望ましい。~~ → **実装済み**。`src/zc/finality_audit.ts` の
  `runFinalityChainAudit` が全チェーンを走査し、断絶検知時は CASE へ収束
  させる。EOD バッチ（`runEod` ステップ 9）に組込み済み。手動実行は
  `POST /internal/cron/finality-audit`。
- **OpenAPI YAML 自動生成**: `src/openapi/*.yaml` は手書き。コードの
  ルーティングと型から再生成する仕組みを入れたい。

規範 > 実装ギャップ（zenith_public.md に規範あり、コードに反映未済）:
- **DNS HOLD の igs_mode 階層遷移**: `igs_mode` 列は存在し `NORMAL → STOP`
  のみ実装。`RINGFENCED` / `RINGFENCED_PLUS` への昇格遷移、原因行集合
  `HOLD_CAUSING_PARTICIPANTS` の管理、`dns_recovery_reserve` 算定、
  `reserve_explain_hash` / `reserve_confidence` 記録、`igs_throttle_budget`
  による公平性制御、Defer キューはすべて未実装。zenith_public.md § 11.4。
- ~~**H_locked 自動解放経路**: `NoDebitRecordedProofSubmitted`
  （未実行証明）と `HUnlockAuthorized`（CASE 4 眼承認）のイベント・API
  が `FinalityEventType` union に未登録。DECIDED_TO_SETTLE 後の長期
  H_locked 詰まりを救済する経路が無い。~~ → **実装済み**。
  `src/zc/h_unlock.ts` に両経路を実装（`submitNoDebitProof` /
  `authorizeHUnlock`）。`POST /api/transfers/:txid/no-debit-proof`
  （X-ZC-Signature 検証）と `POST /api/transfers/:txid/h-unlock-authorize`
  （4 眼 + 証跡必須）。a/b 成立済みは現在状態と FinalityLog 履歴の双方で
  ガードし、解放を拒否（補償＝Reversal 経路へ）。zenith_public.md § 8.4.1。
- **MisrecordCorrected**: ZC 障害による誤記録の訂正経路。未実装
  （`FinalityEventType` 未登録）。zenith_public.md § 13.4。
- **Bulk LSM 流動性節約最適化**: `src/zc/lanes/bulk.ts` は FIFO 相当の
  軽量実装。zenith_public.md § 4.2.4 / 付録 F が規範化する目的関数・
  採択理由証跡・フォールバック（greedy 等）は未実装。
- **GTID N:M fan-in/out**: 1×1 と 2×2（leg_id 辞書順）のみ厳密対応。
  PAYEE 数が PAYER 数より少ない fan-in は「先頭 PAYEE フォールバック」、
  PAYER 数が PAYEE 数より少ない fan-out は未対応。
