...
#### 2.3.1 HTLC Auth（受取側起点オーソリ）の制度上の位置付け【規範】
...
2. **ホワイトリスト（HtlcAuthWhitelist）**: 受取側を加盟店として事前登録する制度（migration 0003）。`allowed_payer_bank_id` / `max_amount` / `allowed_purposes` 等で適用範囲を絞る。**ホワイトリスト登録は ZC 運営の制度行為**で、参加行が無制限に他行顧客に対するオーソリを発行できないようにする目的。
3. **承認責任**: 加盟店審査（KYB 相当）の責任は ZC 運営 + 受取側参加行に集約する（与信・反社チェック等）。
...
8. **PAYER ↔ PAYEE 対応付け規範** ：レッグ間の対応関係は **`leg_id` の辞書順位** で確定する。`INSERT` 順や DB ROWID に依存した対応付けは、取り違いの着金（A→B を A→C と誤実行）を招くため禁止する。実装は PAYER 配列・PAYEE 配列をそれぞれ `leg_id` 昇順にソートし、同一 index 同士で対をなす（PAYEE 数が PAYER 数より少ない fan-in は先頭 PAYEE フォールバック）。本文 § 4.2.5 の規範と一致。
...
### 2.7 HIGH_VALUE 自動エスカレーション閾値のガバナンス【規範】

`Participants.hv_threshold`（migration 0020）を変更することで、`EXPRESS` /
`STANDARD` で指定された送金を `HIGH_VALUE` レーンへ強制ルーティングする金額
閾値が決まる。これは「即時性 vs リスク統制」のトレードオフを統制側に倒す
**制度的判断**であり、技術設定ではない。
...
3. **既定値**:
   ...
   - 最終フォールバック: 1 億円（`DEFAULT_HV_THRESHOLD = 100_000_000`）。
4. **適用効果（規範）**: 受付時に `body.lane` を `HIGH_VALUE` に書換え、
   FinalityLog の `PaymentInitiated` イベントには書換え後の lane が記録され
   る。利用者・参加行は照会 API（`/api/transactions/:txid`）で書換えを
   観測できる。API レスポンス本文ではなく `state` 遷移経路（EXPRESS の
   `H_RESERVED` ではなく HIGH_VALUE の `PRECHECKED → DECIDED_TO_SETTLE`
   経路）で識別する。
5. **禁止事項**:
