...
> <b>設計規範（システミック領域の取り違え事故抑止）</b><br/>
> 利用者・参加行が <code>lane=EXPRESS</code> または <code>lane=STANDARD</code> を指定して送金しても、<b>金額が閾値以上の場合は ZC が受付時点で lane を HIGH_VALUE に書き換える</b>。これは「即時性 vs リスク統制」のトレードオフを<b>制度規範として統制側に倒す</b>判断であり、ユーザー側の意図に依存しない。

- **閾値の優先順位**:
  1. 参加行ごとの個別閾値 `Participants.hv_threshold`（migration 0020）
  2. 環境変数 `ZC_HV_THRESHOLD`
  3. デフォルト 1 億円（`DEFAULT_HV_THRESHOLD = 100_000_000`）

- **エスカレーションの効果**: 受付時に `body.lane = 'HIGH_VALUE'` に書換え、以降 `HIGH_VALUE` レーンの規範フロー（§4.2.6）に従う。FinalityLog の `PaymentInitiated` イベントには書換え後の lane が記録される。

- **ガバナンス**: 閾値変更は ZC 運用責任者（OPS）が行い、変更履歴は監査対象。参加行ごとの閾値は参加契約の付帯条件として明示する。<b>運用が一方的に閾値を引き下げて参加行の即時性をスポイルすることは不可</b>。閾値変更時の事前通知期間は政策で定める（実装は規範対象外）。

- **API の見え方**: 利用者は `POST /api/transfers` のレスポンスで自分のリクエストとは異なる state（`H_RESERVED` ではなく `RECEIVED` のままで HIGH_VALUE フローへ）を観測しうる。API 仕様にこの「lane の書換え可能性」を明記する（`api-contracts.md § POST /api/transfers`）。

- **実装根拠**: `src/zc/ingress.ts` `DEFAULT_HV_THRESHOLD = 100_000_000` 周辺。
...
>顧客向け表示は参加主体が行い、必要に応じて以下のように<b>ぼかしたテンプレート</b>を用いる。
...
> <b>規範（Decision 確定前の不変条件）</b><br/>
> 以下 2 点は GT-level Decision（<code>GT_PRECHECKED → GT_DECIDED_TO_SETTLE</code>）<b>確定前</b>に検証し、満たさない場合は <code>GT_DECIDED_CANCEL</code> に収束させる。Decision 確定後にこれらの整合性違反を発見した場合、不正な遷移が FinalityLog に残り監査説明が破綻する。<br/>
> <b>1. 金額均衡（PAYER 総額 == PAYEE 総額）</b>：<code>sum(PAYER leg amounts) == sum(PAYEE leg amounts)</code>。違反は <code>reason_code=AMOUNT_BALANCE_MISMATCH</code> で取消。<br/>
> <b>2. ロール完全性（PAYER と PAYEE が共に存在）</b>：片側のみの leg 集合は禁止（<code>reason_code=MISSING_LEG_ROLE</code>）。<br/>
> <b>PAYER ↔ PAYEE 対応付け規範</b>：レッグ間の対応関係は <b><code>leg_id</code> の辞書順位</b>で確定する（PAYER 配列・PAYEE 配列をそれぞれ <code>leg_id</code> ソートし、同一 index で対をなす）。<code>INSERT</code> 順や <code>ROWID</code> 順に依存すると取り違いの着金（A→B を A→C と誤実行）を招く。PAYEE 数が PAYER 数より少ない場合は先頭 PAYEE をフォールバックとする（fan-in 利便性のため）。<br/>
> 実装根拠: <code>src/zc/lanes/gtid.ts#advanceGtid</code>（Bug #2 / Bug #6 fix）、テスト: <code>test/integration/balance_invariants.test.ts</code>（GTID 2×2 逆順ケース）。
...
> <b>モック実装状況（規範との差分）</b><br/>
> 本リポジトリ（`zenith-mock`）における現状の実装は、上記規範のうち以下のみ充足する：<br/>
> ・<b>`DnsCycles.igs_mode` 列</b>は存在し、初期値 <code>NORMAL</code>、`holdDns()` 呼出時に <code>STOP</code> へ遷移する（`src/zc/dns.ts`）。<br/>
> ・<b>`RINGFENCED` / `RINGFENCED_PLUS` への昇格遷移、`dns_recovery_reserve` 算定、`reserve_explain_hash`、`reserve_confidence`、`igs_throttle_budget`、`HOLD_CAUSING_PARTICIPANTS` セット管理</b>は<b>未実装</b>（規範のみ）。<br/>
> ・原因行集合の特定、Accept 条件の段階的緩和、Defer キューもモックでは保持しない。<br/>
> 規範を実装側に反映する作業は `architecture.md § 7` のロードマップ項目として別途追跡する。
...
- **高額即時レーン（HIGH_VALUE）のみ**：
  - `PRECHECKED → DECIDED_TO_SETTLE` 直行を許容する（H 予約スキップ。§ 1.2.3 参照）。実装上は `src/zc/orchestrator/state_machine.ts#ALLOWED_TRANSITIONS.PRECHECKED` に明示列挙し、HV 以外のレーンは `transitionWithLog({ fromState: 'H_RESERVED', ... })` で `fromState` を縛ることで間接的に禁止する。
  - `reason_code ∈ {SUSPEND_IGS_HOLD, SUSPEND_IGS_PENDING}` で `SUSPENDED` にある取引について、ExternalSettlementAdapter から `EVT_CB_SETTLED`（中銀決済完了）を受信した場合に限り、`SUSPENDED → PAYEE_EXEC_CONFIRMED` を許容する。
- **HTLC / HTLC_AUTH のみ**：`RECEIVED → HTLC_LOCKED` を canonical 入口とする。`HTLC_LOCKED` で直接 INSERT する経路は禁止し、`insertTxWithLog({ initialState: 'RECEIVED', ... })` で `RECEIVED` を経由してから `transitionWithLog` で `HTLC_LOCKED` に遷移する（`PaymentInitiated` の FinalityLog 痕跡を残すため。背景は `zenith_policy.md` § 2.3.1）。
...
> <b>モック実装状況（規範との差分）</b><br/>
> 本リポジトリでは <code>HReservationPlaced</code> / <code>DECIDED_TO_SETTLE</code>（<code>lockH</code>）/ <code>DECIDED_CANCEL</code>（<code>releaseH</code>）/ <code>DNS_CYCLE_SETTLED</code>（DNS settle 時の一括解放）の 4 トリガのみ実装している（`src/zc/h_model.ts`, `src/zc/dns.ts`）。<br/>
> <b><code>NoDebitRecordedProofSubmitted</code></b>（未実行証明に基づく自動解放）と <b><code>HUnlockAuthorized</code></b>（CASE 4 眼による解放）は <b>FinalityEventType union 未登録 / 専用 API 未実装</b>で、規範のみとして残置している。後者 2 トリガは「DECIDED_TO_SETTLE 後に長期間 a が成立しない」H_locked 詰まりの解消経路であり、本格運用化前に実装が必要。<br/>
> 同じく <b><code>MisrecordCorrected</code></b>（誤記録訂正、§ 13.4）も <b>未実装</b>。
...
#### 10.2.1 ハッシュチェーン（規範：改ざん検知）

> <b>設計規範</b><br/>
> FinalityLog は **WORM（追記専用）かつ tamper-evident**。同一 chain 内の各エントリは SHA-256 で直前エントリの `entry_hash` を参照する。chain identifier は <code>COALESCE(txid, gtid, 'GLOBAL')</code>。

- **アルゴリズム**: `SHA-256 hash-chain v1`（`algorithm` フィールドで明示）
- **entry_hash の決定式**:
  ```
  entry_hash = SHA-256(
  ...
  ```
  パイプ `|` 連結。フィールド順は契約（プロトコル変更不可、必要時は v2 として別アルゴリズム名で導入）。
...
- **event_seq の単調性（規範）**: `FinalitySeq.next_seq` を `UPDATE ... RETURNING` でアトミック増分し、その戻り値を `event_seq` として割当てる（migration 0021、`src/zc/orchestrator/finality.ts#allocateEventSeq`）。旧方式（`Date.now()*1000 + random` + UNIQUE 衝突 retry）は廃止。シード行（`id=1`）が未投入の環境では `MAX(event_seq)+1` を defensive bootstrap として返す。
  - 全体 UNIQUE 索引 `idx_fl_event_seq_unique`（migration 0018 B6）は **belt-and-braces**（カウンタが冪等に動かない事象に対する保険）であり、主たる単調性保証ではない。
...
- **break_reason 区分**（検証失敗の理由）:
  - `LEGACY_UNCHAINED_ENTRY` — migration 0015 以前の旧データ（`entry_hash IS NULL`）
  - `PREV_HASH_MISMATCH` — チェーン分岐（部分 UNIQUE 索引 B5/B9 が事前に防ぐ）
  - `ENTRY_HASH_MISMATCH` — 内容書き換え（DB 直接編集等）

- **並列書込みの安全性（規範）**: 同 chain への並列 INSERT は部分 UNIQUE
  索引（`idx_fl_chain_prev_hash`：migration 0018 B5、TX チェーン用 ／
  `idx_fl_gtid_chain_prev_hash`：migration 0019 B9、GTID 専用チェーン用）で
...
- 実装: `src/zc/explain.ts`, `src/zc/story.ts`, `src/zc/finality_chain.ts`。
...
参加行ごとに以下のカウンタを保持する（migration 0017、`schema.md § CircuitBreakerState`）：
...
- **reason 区分** (`ReversalRecords.reason`):
  - `CUSTOMER_DISPUTE` — 顧客からの異議
  - `DUPLICATE_PAYMENT` — 二重送金（システム的に冪等で吸収できなかった残存）
  - `FRAUD` — 不正取引
  - `WRONG_RECIPIENT` — 誤受取人
  - 等（実装の `REVERSAL_REASONS` を正とする）

- **事前承認必須の reason（規範）**: `FRAUD` など一部の reason は内部統制系の事前承認チケット参照（`approval_ref`）の付与が必須。実装上は `APPROVAL_REQUIRED_REASONS` セットに列挙し、欠落時は `422 APPROVAL_REQUIRED` を返す（migration 0018 B4 で `ReversalRecords.approval_ref` 追加）。
