...
| 次に動けるまで | 次の再振替サイクル待ち（約 16 日） | その場で「いま払う」を選べる |
| 問い合わせ | 各自が状況を別々に再構成 | 全員が同じ取引番号・同じ時刻・同じ理由コードを見る |

短くなるのは **「気づく」「次に動ける」までの時間** であって、Zenith が入金そのものを肩代わりするわけではありません（成立は利用者の資金手当次第）。派手な革新ではなく、**ただ「説明できる状態の連なり」がそこにある**だけ。詳細は [`specs/walkthrough.md`](specs/walkthrough.md)（5 分）。
...
ここで並べた原始要素は、どれも新発明ではありません。追記専用ログ＋状態機械＋遷移のアトミック性は許可型台帳や RTGS のグロス決済が、HTLC をレーンとして扱う発想は Interledger / Rafiki が、すでに体現しています。Zenith の貢献はもっと控えめで、**それらを橋渡しではなくひとつの状態機械の上に統合し、制度文書まで含めて最後まで動くオープンソースの合成として示した** 点に絞られます。具体的には次の三点です。

- **HTLC・GTID 原子マルチレグ・RTGS・DNS ネッティングを、橋渡しではなく、ひとつの状態機械の対等なレーンとして並べて動かしたこと。**
- **状態遷移と FinalityLog 書込みのアトミック強制、および単一 trace ID による生涯トレースを、テストで規約として強制したこと。**
- **DNS_HOLD 時の流動性供給カスケードを、コードと同じ温度の規程文書として書き切ったこと**（[`specs/zenith_policy.md`](specs/zenith_policy.md)）。
...
**A 30-second example (a household direct debit that fails on insufficient funds):** today, the customer learns of the failure days later by mail and the next retry is ~16 days away on the institutional re-debit cycle; with Zenith, both customer and biller see the same notification within seconds and the payer can choose to act immediately, all reading **the same transaction id, the same timestamps, the same reason code**. What shrinks is the time to *awareness* and the time until the payer can *act* — not the funding itself (final settlement still depends on the customer covering the balance). See [`specs/walkthrough.md`](specs/walkthrough.md).
...
Most cited precedents (BIS Stella, Jasper-Ubin, Agorá, mBridge, Partior, Fnality, JPM Onyx, Canton, DCJPY / Progmat) reach into the **monetary substrate** — tokenised deposits, wholesale CBDC, a unified ledger. Zenith does the opposite: it takes commercial-bank money and existing ledgers as given and designs the **layer on top**. Adjacent layers, not competitors. None of the primitives here is itself novel — permissioned ledgers and RTGS gross settlement already embody an append-only log plus a state machine plus atomic transitions, and treating HTLC as a lane is the Interledger / Rafiki idea. Zenith's contribution is narrower: a runnable open-source **synthesis** that puts them on one state machine, institutional rulebook included, rather than joining ledgers by a bridge. Concretely: (1) HTLC, GTID atomic multi-leg, RTGS, and DNS netting as coequal lanes inside one state machine; (2) atomic pairing of state transitions with FinalityLog writes plus single-trace-id lifecycle tracing, enforced as a convention by tests; (3) a liquidity-cascade rulebook for DNS_HOLD written at the same fidelity as the code ([`specs/zenith_policy.md`](specs/zenith_policy.md)). Tokenised deposits, wholesale CBDC, cross-border multi-currency atomicity, ZKP privacy, and agent identity are deliberately **out of scope**.
