...
この同型性は、ホールセール CBDC・トークン化預金・共通台帳といった下層に関心を持つ読者にとって、ひとつの示唆を含みます。すなわち、**「下層の決済通貨をパブリックチェーンに wrap する」（Lock and Mint）と「発行主体が自前で許可型チェーンを立てる」では、難易度の方向がまったく違う** ということです。

前者は、ファイナリティモデルの不整合（即時・法的ファイナル vs 確率的ファイナル）と、ブリッジ保管リスク（Ronin・Wormhole・Nomad の前例）という、**立法では消せない問題** を抱え込みます。wrap された瞬間、そのトークンは下層の決済通貨そのものではなく、ロックされた準備に対する私的請求権に変わります。
...
For readers thinking about the layer below — wholesale CBDC, tokenised deposits, a unified ledger — this isomorphism carries one implication. **Wrapping the substrate money onto a public chain (lock-and-mint), and the issuing authority running its own permissioned chain, are difficulties pointing in opposite directions.**

The former inherits a finality-model mismatch (instant, legally-final settlement vs. probabilistic finality) and the bridge-custody risk visible in the Ronin, Wormhole, and Nomad cases — problems that legislation cannot remove. The moment wrapping occurs, the token ceases to be the substrate money itself and becomes a private claim against locked reserves.
